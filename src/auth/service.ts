...
export class AuthService {
  ...
  async authenticate(token: string | undefined): Promise<User | undefined> {
    ...
    return this.decodeToken(token).then(
      (user) => user && this.getOrCreateUser(user),
    );
  }

  async decodeToken(token: string): Promise<FirebaseUser | undefined> {
  ...
  async getOrCreateUser(firebaseUser: FirebaseUser): Promise<User | undefined> {
